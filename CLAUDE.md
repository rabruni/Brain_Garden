...
Dual-project monorepo:

1. **HRM_Test/** — Hierarchical Reasoning Model: a recurrent neural network (27M params) for abstract reasoning (ARC puzzles, Sudoku, Mazes) trained on 1000 examples
2. **Control_Plane_v2/** — Multi-tier governance infrastructure with package management, integrity verification, and auditable agent coordination

## HRM_Test

### Prerequisites
- CUDA 12.6 + PyTorch with CUDA support
- FlashAttention 3 (Hopper GPUs) or FlashAttention 2 (Ampere/earlier): `pip3 install flash-attn`
...
### Architecture
Two interdependent recurrent modules in `models/`:
- **H (High-level)**: Slow, abstract planning (H_cycles iterations)
- **L (Low-level)**: Fast, detailed computation (L_cycles iterations)

Key files: `models/hrm/hrm_act_v1.py` (main model with ACT + halting Q-learning), `models/layers.py` (FlashAttention, RoPE, SwiGLU, RMS norm), `models/losses.py` (Stablemax cross-entropy, ACT loss head). Config via Hydra/OmegaConf in `config/`.

## Control_Plane_v2

### Tier Layout
- **HOT/** — Executive/governance tier: kernel libs, scripts, schemas, governance ledger
- **HO3/** — Learning tier: spec packs, operational scripts, package ledger, installed packages
- **HO2/** — Session tier
- **HO1/** — Fast/stateless tier

### Package Workflow
```bash
# Pack a package
python3 scripts/package_pack.py --src <path> --id PKG-<NAME> --frameworks-active FMWK-107 --session S1 --work-order WO1 --actor you --token <token>
...
python3 HOT/scripts/cp_version_checkpoint.py --label "label" --token <token>
python3 HOT/scripts/cp_version_list.py
python3 HOT/scripts/cp_version_rollback.py --version-id VER-... --token <token>
```

### Testing
```bash
pytest Control_Plane_v2/HOT/tests/ -v
pytest Control_Plane_v2/HO3/tests/ -v
```

### Key Libraries (HOT/kernel/)
...
- `integrity.py` / `merkle.py` — Integrity and Merkle verification
- `auth.py` / `authz.py` — Pluggable auth (passthrough/HMAC), role-based access
- `provenance.py` — Source tracking and dependency resolution

### Environment Variables
- `CONTROL_PLANE_AUTH_PROVIDER`: `passthrough` (dev) or `HMAC` (production)
