...
DEFAULT_PACKAGE_GROUPS: Dict[str, PackageGroupConfig] = {
    # Shell module - uses existing SPEC-SHELL-001 manifest assets
    # Framework: FMWK-SHELL-001 (needs registration)
    "PKG-SHELL-001": PackageGroupConfig(
        package_id="PKG-SHELL-001",
        spec_id="SPEC-SHELL-001",
        framework_id="FMWK-SHELL-001",  # Correct framework for shell
        title="Universal Shell Module",
        ...
        invariants=["Shell commands MUST be logged to ledger"],
        acceptance_tests=["pytest tests/test_shell*.py -v"],
    ),

    # Chat interface - uses existing SPEC-CHAT-001 manifest assets
    # Framework: FMWK-CHAT-001 (needs registration)
    "PKG-CHAT-001": PackageGroupConfig(
        package_id="PKG-CHAT-001",
        spec_id="SPEC-CHAT-001",
        framework_id="FMWK-CHAT-001",  # Correct framework for chat
        title="Chat Interface Module",
        ...
        invariants=["All LLM calls MUST be logged"],
        acceptance_tests=["pytest tests/test_chat*.py -v"],
    ),
...
from lib.registry import framework_exists as check_framework_exists  # canonical lookup
from lib.registry import spec_exists as check_spec_exists  # canonical lookup
...
def execute_remediation(
    ...
    if install and created_packages and not dry_run:
        ...
        for config, staging_dir in created_packages:
            ...
            # Create archive using lib/packages.pack()
            from lib.packages import pack
            archive_path = ROOT / "_staging" / f"{config.package_id}.tar.gz"
            print(f"    Creating archive...")

            archive_hash_raw = pack(staging_dir, archive_path)
            archive_hash = f"sha256:{archive_hash_raw}"
