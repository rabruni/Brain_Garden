...
def _load_external_secrets() -> dict[str, str]:
    ...
    Note (Issue 4 Fix):
        When CONTROL_PLANE_SECRETS_FILE is set, we MUST NOT import kernel.paths
        to avoid bootstrap recursion. Only import kernel.paths when searching
        default paths that need CONTROL_PLANE.
    """
    # Check explicit path first - MUST NOT import kernel.paths here (Issue 4)
