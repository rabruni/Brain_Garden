...
frozen dataclasses. All path lookups go through the Layout singleton:
    ...
    LAYOUT.tier("HOT").installed   # → .../HOT/installed
    LAYOUT.registry_file("file_ownership")   # → .../HOT/registries/file_ownership.csv
    LAYOUT.ledger_file("HOT", "packages")    # → .../HOT/ledger/packages.jsonl
"""
...
@dataclass(frozen=True)
class TierLayout:
    """Resolved paths for a tier (HOT, HO2, HO1)."""
    root: Path
    registries: Path
    installed: Path
    ledger: Path
...
class Layout:
    ...
    def tier(self, name: str) -> TierLayout:
        """Get resolved paths for a tier (HOT, HO2, HO1).

        Args:
            name: Tier name (HOT, HO2, HO1)
