...
def load_governed_roots(plane_root: Path) -> dict:
    ...
    if hot_config_path.exists():
        with open(hot_config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        # Use hot governed roots
        roots = config.get('hot_governed_roots', [])
        if roots:
    ...
    return {
        ...
        "excluded_patterns": ["**/__pycache__/**", "**/*.pyc", "**/__init__.py", "**/file_ownership.csv", "**/packages_state.csv"]
    }
...
def check_g0b_plane_ownership(plane_root: Path) -> GateResult:
    ...
    if not ownership:
        ...
        warnings.append("Run: python3 scripts/rebuild_derived_registries.py --plane hot")
    ...
    for file_path, entry in ownership.items():
        ...
        expected_hash = entry.get('sha256', '').removeprefix('sha256:')
        if expected_hash:
            actual_hash = compute_sha256(full_path).removeprefix('sha256:')
    ...
    if missing:
        result.passed = False
        errors.extend([f"MISSING: {p} (owned by {ownership[p].get('package_id', '?')})" for p in missing[:10]])
        if len(missing) > 10:
...
def check_g1_chain(plane_root: Path) -> GateResult:
    ...
    1. If spec_id present: verify spec exists in specs_registry.csv
    2. For each spec: verify framework_id exists in frameworks_registry.csv
    3. Packages without spec_id get warnings (Layer 0 axioms)
    """
    ...
    if specs_path.exists():
        with open(specs_path, 'r') as f:
            for row in csv.DictReader(f):
                sid = row.get('spec_id', '').strip()
                if sid:
                    specs_by_id[sid] = row
    else:
        warnings.append("specs_registry.csv not found")

    # Read frameworks_registry
    if fmwk_path.exists():
        with open(fmwk_path, 'r') as f:
            for row in csv.DictReader(f):
    ...
    # Find all installed package manifests
    # Check both tier-level (HOT/installed/) and root-level (installed/)
    manifests_found = 0
    seen_pkg_ids = set()
    installed_dirs = []
    for tier in ['HOT', 'HO2', 'HO1']:
        d = plane_root / tier / 'installed'
        if d.is_dir():
            installed_dirs.append(d)
    root_installed = plane_root / 'installed'
    if root_installed.is_dir():
        installed_dirs.append(root_installed)
    ...
    for installed_dir in installed_dirs:
        for pkg_dir in sorted(installed_dir.iterdir()):
            ...
            pkg_id = manifest.get('package_id', pkg_dir.name)

            # Deduplicate: same package may appear in tier + root installed dirs
            if pkg_id in seen_pkg_ids:
                continue
            ...
            if not spec_id:
                warnings.append(f"NO_SPEC: {pkg_id} has no spec_id (Layer 0 axiom)")
                continue

            # Validate spec exists
            if spec_id not in specs_by_id:
                errors.append(f"SPEC_NOT_FOUND: {pkg_id} -> {spec_id} not in specs_registry.csv")
                continue

            # Validate framework exists
            framework_id = specs_by_id[spec_id].get('framework_id', '').strip()
            if not framework_id:
                errors.append(f"NO_FRAMEWORK: spec {spec_id} has no framework_id")
                continue

            if framework_id not in fmwks_by_id:
                errors.append(f"FMWK_NOT_FOUND: {pkg_id} -> {spec_id} -> {framework_id} not in frameworks_registry.csv")
                continue
    ...
    if errors:
        result.passed = False
        result.message = f"G1 FAILED: {len(errors)} chain errors"
        result.errors = errors[:20]
    else:
        result.message = f"G1 PASSED: {chains_validated} chains validated, {len(warnings)} warnings"

    result.warnings = warnings
    result.details = {
        "manifests_found": manifests_found,
        "chains_validated": chains_validated,
        "specs_loaded": len(specs_by_id),
        "frameworks_loaded": len(fmwks_by_id),
    }
...
def check_g1_complete(plane_root: Path) -> GateResult:
    """G1-COMPLETE: FRAMEWORK COMPLETENESS - Verify framework wiring diagram is satisfied.

    For every installed package with a framework_id:
    1. Framework must have expected_specs declared
    2. Every expected_spec must exist in spec_packs/
    3. Every expected_spec must reference this framework back
    """
    result = GateResult(gate="G1-COMPLETE", passed=True, message="Framework completeness check passed")
    errors = []
    warnings = []
    ...
    try:
        ...
        return GateResult(
            ...
            warnings=["kernel.preflight.FrameworkCompletenessValidator not importable"],
        )
    ...
    for installed_dir in installed_dirs:
        for pkg_dir in sorted(installed_dir.iterdir()):
            ...
            vresult = validator.validate(manifest)
    ...
    if errors:
        result.passed = False
        result.message = f"G1-COMPLETE FAILED: {len(errors)} completeness errors"
        result.errors = errors[:20]
    else:
        result.message = f"G1-COMPLETE PASSED: {frameworks_checked} frameworks checked"

    result.warnings = warnings
    result.details = {"frameworks_checked": frameworks_checked}
...
Examples:
    ...
    # Run specific gates
    python3 scripts/gate_check.py --gate G0B G1 G6 --plane hot
"""
    )
    parser.add_argument(
        "--gate", "-g",
    ...
    if args.plane:
        if args.plane == "hot":
            plane_root = args.root  # HOT is the root
        else:
