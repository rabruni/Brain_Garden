...
class AdminTools:
    ...
    def _resolve_path(self, path: str, check_permission: bool = True) -> tuple[Path, str]:
        """Resolve and validate a path.

        Args:
            path: Absolute or relative path
            check_permission: Whether to check read permission (False for dirs)
        ...
        # Check permissions (skip for directory listing)
        if check_permission and not self._is_allowed(rel_path):
    ...
    def list_directory(self, path: str = ".", pattern: str = "*") -> Dict[str, Any]:
        ...
        """
        # Don't check file permission for directory listing
        abs_path, rel_path = self._resolve_path(path, check_permission=False)
    ...
    def glob(self, pattern: str, path: str = ".") -> Dict[str, Any]:
        ...
        """
        # Don't check file permission for directory itself
        abs_path, rel_path = self._resolve_path(path, check_permission=False)
    ...
    def grep(self, pattern: str, path: str = ".", file_pattern: str = "*.jsonl") -> Dict[str, Any]:
        ...
        Returns:
            Dict with matches
        """
        # Don't check permission for directory itself (files checked individually)
        abs_path, rel_path = self._resolve_path(path, check_permission=False)
