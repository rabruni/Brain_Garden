...
Proves the router and admin agent work correctly with pure routing:
- Router classifies intent and maps to handler name
- admin_turn dispatches the handler (searching both modules)
- No mode-based dispatch — handler lookup is mode-free

Tests:
1. RouteResult contract invariants on representative inputs
2. All valid intents produce known handlers
3. route_query() does not execute handlers or call classify_intent more than once
4. admin_turn() dispatches exactly the selected handler (no double-call)
5. Handler not found returns error (fail-closed)
6. handler_executed evidence records what handler actually ran
"""
...
# =============================================================================
# Test vectors: (intent, confidence, expected_handler)
# =============================================================================
...
ROUTE_VECTORS = [
    ...
    ("show_session",     0.79, "show_session_ledger"),
    ("read_file",        0.50, "read_file"),
    ("validate",         0.70, "validate_document"),
    ("general",          0.30, "general"),
    ("summarize",        0.60, "summarize"),
    # Unknown intent -> defaults to "general"
    ("nonexistent_xyz",  0.60, "general"),
]
...
class TestRouteResultContract:
    ...
    def _mock_classify(self):
        """Mock classify_intent and gather_capabilities so we control the IntentResult."""
        ...
        with patch("modules.router.prompt_router.classify_intent") as mock_ci, \
            ...
            mock_ci.side_effect = lambda query, capabilities=None: self._mock_intent
    ...
    def test_route_contract(self, intent, confidence, expected_handler):
        """route_query() maps intent to correct handler and returns ROUTED mode."""
        ...
        # C1: mode is always ROUTED from route_query (never DENIED)
...
class TestAdminTurnHandlerDispatch:
    ...
    def test_exactly_one_handler_called(
        ...
        try:
            with patch(
                ...
                "modules.router.decision.gather_capabilities",
                return_value=None,
            ):
    ...
    def test_no_fallback_when_handler_exists(self, tmp_path):
        """When handler exists, no other handler is called."""
        _setup_cp_dirs(tmp_path)
    ...
    def test_missing_handler_fails_closed(self, tmp_path):
        """Handler missing from both modules returns error, no fallback."""
        _setup_cp_dirs(tmp_path)

        # Force INTENT_HANDLER_MAP to return a quarantined handler name
        mock_intent = IntentResult(
            intent="list_packages",
            confidence=0.95,
    ...
    def test_missing_handler_records_denied_in_evidence(self, tmp_path):
        ...
        with patch(
            ...
            "modules.router.decision.INTENT_HANDLER_MAP",
            {"list_packages": "inventory"},  # Force quarantined handler
        ), patch(
...
class TestHandlerExecutedEvidence:
    ...
    def test_handler_executed_present_and_matches(
        ...
        with patch(
            ...
            f"modules.admin_agent.handlers.{handler_module}.{expected_handler}",
        ...
        assert he["mode"] == "routed"
        assert he["executed"] is True
        assert "timestamp" in he
        assert he["authorization"] == "direct"
    ...
    def test_handler_executed_false_when_denied(self, tmp_path):
        ...
        assert he["handler"] == "inventory"
        assert he["executed"] is False
        assert he["mode"] == "denied"
        assert he["authorization"] == "none"

    def test_handler_executed_confirmation_id_always_none(self, tmp_path):
        """handler_executed.confirmation_id is always None (confirmation gate removed)."""
        _setup_cp_dirs(tmp_path)
...
class TestQuarantineUnmappedHandlers:
    ...
    @pytest.mark.parametrize(
        "handler_name",
        QUARANTINED_HANDLERS,
        ids=QUARANTINED_HANDLERS,
    )
    def test_quarantined_handler_returns_none(self, handler_name):
    ...
    @pytest.mark.parametrize(
        "handler_name",
        ROUTABLE_HANDLERS,
        ids=ROUTABLE_HANDLERS,
    )
    def test_routable_handler_returns_callable(self, handler_name):
...
class TestErrorPathEvidencePreservation:
    ...
    def test_error_before_routing_has_no_route_decision(self, tmp_path):
        ...
        with patch(
            "modules.router.prompt_router.classify_intent",
            side_effect=RuntimeError("classify exploded"),
        ), patch(
            "modules.router.decision.gather_capabilities",
            return_value=None,
        ), patch(
        ...
        # No route_decision since error was before routing completed
...
class TestAuthorizationFieldInEvidence:
    """handler_executed.authorization is always 'direct' for read-only handlers."""

    def test_direct_authorization(self, tmp_path):
        """All queries -> authorization='direct' (confirmation gate removed)."""
        _setup_cp_dirs(tmp_path)

        mock_intent = IntentResult(
            intent="list_packages",
            confidence=0.95,
            reasoning="test direct auth",
        )
...
class TestPreRouterNoSideEffects:
    ...
    def test_no_subprocess_before_route_query(self, tmp_path):
        ...
        def spy_classify_intent(query, capabilities=None):
        ...
        with patch("subprocess.run", side_effect=spy_subprocess_run), \
             ...
             patch("modules.router.decision.gather_capabilities",
                   return_value=None), \
...
class TestNonMockProviderResponse:
    """LLM_ASSISTED_PROVIDER env var flows through to complete() provider_id."""
    ...
    def test_general_handler_with_stub_provider(self, tmp_path, monkeypatch):
        ...
        mock_response = LLMResponse(
            content="Hello! I'm the Control Plane Admin Agent. How can I help you today?",
            model="stub-model",
        ...
        with patch("modules.admin_agent.handlers.llm_assisted.complete",
                   return_value=mock_response) as mock_complete, \
             patch("modules.admin_agent.handlers.llm_assisted.load_governed_prompt",
        ...
        assert mock_complete.call_args[1]["prompt_pack_id"] == "PRM-ADMIN-GENERAL-001"

        # Output is conversational, not brain advisory metadata
        assert "Mock response" not in result
        assert "Hello" in result or "Admin Agent" in result
...
class TestToolsFirstReadOnlyExecution:
    """Tools-first handlers execute directly — they are read-only observations."""
    ...
    def test_tools_first_handler_executes_and_returns_content(
        ...
        """Handlers execute directly and return real content (not errors)."""
        _setup_cp_dirs(tmp_path)
...
class TestEvidenceCompleteness:
    """Evidence contains required route_decision fields."""

    def test_evidence_has_route_decision(self, tmp_path):
        """Handler produces evidence with route_decision fields."""
        _setup_cp_dirs(tmp_path)

        mock_intent = IntentResult(
            intent="list_packages",
            confidence=0.95,
            reasoning="test evidence completeness",
            provider_id="mock",
        )
