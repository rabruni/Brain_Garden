...
class TestAdminTurnWithRouter:
    ...
    def test_list_packages_tools_first(self):
        """List packages uses tools-first mode (direct execution)."""
        result = admin_turn("What packages are installed?", root=CONTROL_PLANE)
        assert "Installed" in result or "packages" in result.lower() or "PKG-" in result

    def test_explain_tools_first(self):
        """Explain uses tools-first mode (direct execution)."""
    ...
    def test_health_check_tools_first(self):
        """Health check uses tools-first mode (direct execution)."""
    ...
    def test_inventory_tools_first(self):
        """Inventory uses tools-first mode (direct execution)."""
...
class TestGetHandler:
    ...
    def test_get_tools_first_handler(self):
        """Get handler from tools_first module."""
        handler = get_handler("list_installed")
        assert handler is not None
        assert callable(handler)

    def test_get_llm_assisted_handler(self):
        """Get handler from llm_assisted module."""
        handler = get_handler("validate_document")
        assert handler is not None
        assert callable(handler)
...
class TestRouterIntegration:
    ...
    def test_route_validate(self):
        """Router routes validate (capabilities ignored, always routed)."""
        result = route_query("Validate document")
        assert result.mode == RouteMode.ROUTED
        assert result.handler == "validate_document"

    def test_route_summarize(self):
        """Router routes summarize (capabilities ignored, always routed)."""
