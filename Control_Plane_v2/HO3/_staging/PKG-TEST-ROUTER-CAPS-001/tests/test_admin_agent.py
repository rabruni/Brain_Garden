...
class TestAdminTurn:
    ...
    Confirmation gate was removed — all read-only handlers execute directly.
    """

    def test_explain_query(self, tmp_path):
        """admin_turn handles explain queries (direct execution)."""
        result = admin_turn(
            "Explain FMWK-000",
            session_id="SES-test",
            turn_number=1,
    ...
    def test_list_query(self, tmp_path):
        """admin_turn handles list queries (direct execution)."""
        result = admin_turn(
            "list packages",
        ...
        assert "Package" in result or "Installed" in result or "PKG-" in result

    def test_status_query(self, tmp_path):
        """admin_turn handles status queries (direct execution)."""
        result = admin_turn(
            "check health",
